...
const Tables = () => {
    ...
    const moveTable = useCallback(async (id, left, top) => {
        const gridX = Math.round(left / cellSize);
        const gridY = Math.round(top / cellSize);
        const centerOffset = (cellSize - tableSize) / 2;
        const newPosition = { x: gridX * cellSize + centerOffset, y: gridY * cellSize + centerOffset };
        ...
        const [{ isDragging }, drag] = useDrag({
            ...
            collect: (monitor) => ({
                ...
                if (delta && containerRect) {
                    ...
                    const boundedLeft = Math.max(0, Math.min(left, containerRect.width - tableSize));
                    const boundedTop = Math.max(0, Math.min(top, containerRect.height - tableSize));
                ...
                <div className="flex flex-wrap items-center mb-4">
                    ...
                    <label className="mr-2 mb-2 flex items-center">
                        <input
                            ...
                            onChange={(e) => setNewTableBookable(e.target.checked)}
                            className="mr-1"
                        />
                        Bookable
                    </label>
